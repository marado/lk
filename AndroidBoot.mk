...
ifeq ($(BOOTLOADER_LLVM_SUPPORT),true)
CROSS_COMPILE_CLANG := $(CROOT_DIR)/$(SOONG_LLVM_PREBUILTS_PATH)
LLVM_SUPPORT := LLVM_SUPPORT=1
else
LLVM_SUPPORT := LLVM_SUPPORT=0
endif
...
ifeq ($(PRODUCT_SUPPORT_VIRTUAL_AB_OTA),true)
  VIRTUAL_AB_OTA_SUPPORT := VIRTUAL_AB_OTA_SUPPORT=1
else
  VIRTUAL_AB_OTA_SUPPORT := VIRTUAL_AB_OTA_SUPPORT=0
endif
...
ifeq ($(TARGET_SUPPORTS_WEAR_ANDROID),true)
  ENABLE_SUPPORTS_WEARABLES_TZ_SIZE_OPTI := ENABLE_SUPPORTS_WEARABLES_TZ_SIZE_OPTI=1
endif
...
ifneq ($(TARGET_KERNEL_VERSION), 4.14)
  ENABLE_KPI_PARAM := ENABLE_KPI_PARAM=1
endif
...
$(TARGET_ABOOT_ELF): $(ABOOT_CLEAN) | $(ABOOT_OUT)
	$(MAKE) -C $(LK_PATH) TOOLCHAIN_PREFIX=$(CROSS_COMPILE) CROSS_COMPILE_CLANG=$(CROSS_COMPILE_CLANG) BOOTLOADER_OUT=$(CROOT_DIR)/$(ABOOT_OUT) $(BOOTLOADER_PLATFORM) $(EMMC_BOOT) $(SIGNED_KERNEL) $(VERIFIED_BOOT) $(VERIFIED_BOOT_2) $(VB1_KEY_USED) $(TARGET_DTBO_NOT_SUPPORTED) $(ENABLE_DISPLAY) $(ENABLE_KASLRSEED) $(ENABLE_BOOTDEVICE_MOUNT) $(DEVICE_STATUS) $(BUILD_VARIANT) $(BOARD_NAME) $(ENABLE_VB_ATTEST) $(OSVERSION_IN_BOOTIMAGE) $(QSEECOM_SECAPP_REGION_2MB) $(TARGET_USE_SYSTEM_AS_ROOT_IMAGE) $(DYNAMIC_PARTITION_SUPPORT) $(ENABLE_KPI_PARAM) $(LLVM_SUPPORT) $(ENABLE_SUPPORTS_WEARABLES_TZ_SIZE_OPTI) $(VIRTUAL_AB_OTA_SUPPORT)
...
$(TARGET_NAND_BOOTLOADER): $(appsbootldr_clean) | $(NAND_BOOTLOADER_OUT)
	$(MAKE) -C $(LK_PATH) TOOLCHAIN_PREFIX=$(CROSS_COMPILE)  CROSS_COMPILE_CLANG=$(CROSS_COMPILE_CLANG) BOOTLOADER_OUT=$(CROOT_DIR)/$(NAND_BOOTLOADER_OUT) $(BOOTLOADER_PLATFORM) $(SIGNED_KERNEL) $(BOARD_NAME) $(ENABLE_KPI_PARAM)  $(LLVM_SUPPORT) $(ENABLE_SUPPORTS_WEARABLES_TZ_SIZE_OPTI)
